...
import { Container, Box, Typography, Divider, Grow } from "@mui/material";
import { motion } from "framer-motion";

export default function About() {
  // State to control showing paragraphs
  const [showParagraphs, setShowParagraphs] = useState(false);

  // Trigger paragraphs after a brief delay
  useLayoutEffect(() => {
    const timer = setTimeout(() => setShowParagraphs(true), 600);
    return () => clearTimeout(timer);
  }, []);

  // Framer Motion variants for the title heading
  ...
  return (
    ...
    >
      <Container maxWidth="md">
        {/* Title Animates with Framer Motion */}
        <motion.div
          initial="hidden"
          animate="visible"
        ...
        {/* Paragraphs Fade/Grow In After Delay */}
        <Grow in={showParagraphs} timeout={800}>
          <Box sx={{ color: "var(--color-text-light)", lineHeight: 1.7, fontSize: "1rem" }}>
            ...
            <Typography variant="body1" paragraph>
              Outside of my technocoligal interests I love being active. For the past four years, I’ve been
